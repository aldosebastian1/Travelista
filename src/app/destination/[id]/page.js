...
export async function generateMetadata({ params }) {
    ...
    const pkg = travelPackages.find((p) => p.id.toString() === id);
    ...
    return {
        ...
        openGraph: {
            ...
            images: [{ url: pkg.image, width: 1200, height: 630, alt: `Pemandangan ${pkg.name}` }],
            type: "website",
            locale: "id_ID",
        },
        twitter: {
...
export default async function DestinationDetail({ params }) {
    ...
    const breadcrumbJsonLd = {
        ...
        itemListElement: [
            ...
            { "@type": "ListItem", position: 2, name: "Destinasi", item: `${BASE_URL}/destination` },
            { "@type": "ListItem", position: 3, name: pkg.name, item: `${BASE_URL}/destination/${pkg.id}` },
        ],
    };
    ...
    const infoCards = [
        { Icon: Clock, label: "Durasi", value: "4 Hari", bg: "bg-sky-50", text: "text-sky-500" },
        { Icon: Calendar, label: "Ketersediaan", value: "Sepanjang Tahun", bg: "bg-sky-50", text: "text-sky-500" },
        { Icon: Star, label: "Rating", value: "4.9 / 5.0", bg: "bg-amber-50", text: "text-amber-500" },
    ];
        ...
        <div className="bg-slate-50 min-h-screen pb-20">
            ...
            <nav aria-label="Breadcrumb" className="sr-only">
                <ol>
                    ...
                    <li><Link href="/">Destinasi</Link></li>
            ...
            <div className="relative h-[60vh] min-h-[400px] w-full">
                ...
                <div className="absolute top-24 left-6 md:left-20 z-20">
                    <Link
                        ...
                        className="inline-flex items-center gap-2 text-white bg-slate-900/20 hover:bg-slate-900/50 focus:outline-none focus:ring-2 focus:ring-white backdrop-blur-md px-4 py-2 rounded-full transition-colors font-medium"
                    >
                        <ArrowLeft size={18} aria-hidden="true" />
                        Kembali
                    </Link>
                </div>
                <div className="absolute bottom-0 left-0 w-full z-20 bg-gradient-to-t from-slate-900/80 via-slate-900/40 to-transparent pt-32 pb-12 px-6 md:px-20">
                    <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            <div className="flex items-center text-sky-300 font-medium mb-4">
                        ...
                        <div className="bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-3xl shrink-0">
                ...
                <div className="lg:col-span-2 space-y-12">
                    <section aria-labelledby="about-heading">
                        ...
                        <p className="text-slate-600 leading-relaxed text-lg mb-8">
                            Nikmati pengalaman perjalanan yang tidak terlupakan dengan paket eksklusif {pkg.name}. Kami
                            merancang setiap itinerary dengan teliti untuk memastikan Anda mendapatkan pengalaman terbaik di{" "}
                            {pkg.location} — dengan kenyamanan maksimal dan sentuhan layanan premium kami.
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            {infoCards.map(({ Icon, label, value, bg, text }) => (
                                <div key={label} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
                                    <div className={`p-3 ${bg} ${text} rounded-xl shrink-0`}>
                        ...
                        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {facilities.map((item) => (
                                <li key={item} className="flex items-center gap-3 text-slate-700">
                                    <CheckCircle size={20} className="text-emerald-500 shrink-0" aria-hidden="true" />
                                    <span>{item}</span>
                                </li>
                            ))}
                        </ul>
